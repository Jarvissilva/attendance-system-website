...
const EditClassroom = ({ classroom }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetchApi(
      `/classrooms/${router.query.classroom}/edit`,
      "POST",
      formData
    );
    setMessage(res.message);
            ...
            <table className="w-full">
              ...
              <tbody>
                {formData.students.map((student, index) => (
                  <tr key={index}>
                    <td className="border border-gray-200 sticky left-0 z-20 bg-white whitespace-nowrap">
                      <input
                        type="text"
                    ...
                    <td className="border border-gray-200 sticky left-0 z-20 px-2 bg-white whitespace-nowrap flex justify-between">
                      <input
                        type="text"
                      ...
                      <button
                        ...
                        className="border border-gray-200 px-4 bg-red-500 rounded-md text-white"
                      >
                        <svg
                          fill="#fff"
                          version="1.1"
